...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    temp = data_dict.split(' ')
    data_dict = {}
    for item in temp:
        key,val = item.split('=')
        data_dict[key] = val
    names_dir = data_dict['names']
    with open (names_dir,'r') as f:
        data_dict['names'] = f.readlines()
    ...
    nc, names = (1, ['item']) if opt.single_cls else (int(data_dict['classes']), data_dict['names'])  # number classes, names
